...
class FileService 
{
    ...
    protected function transformObject($object)
    {
        ...
        $array = [
            ...
            'slug' => $object->getDate()->format('Y-m-d') . '-' . StravaService::sanitizeFilename($object->getName())
        ];
    ...
    protected function removeFile($data, $date)
    {
        ...
        $fileName = $this->settings['create']['path'] . '/' . $data['slug'] . '.' . $this->settings['create']['extension'];
        unink($fileName);
        if ($this->settings['commit']['enabled'] === true) {
            $this->commit('Delete Activity "' . $data['slug'] . '"');            
        }
    }
